...
from ..util.utils_squad import convert_examples_to_crops
from ..util.utils_nq import read_nq_examples
...
class NQDatasetReader(DatasetReader):
    ...
    def __init__(self,
        ...
        self.is_training, self.is_validation, self.is_testing = \
            (self.stage == 'training', self.stage == 'validation', self.stage == 'testing')
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        :return:
        """
        examples_gen = read_nq_examples(file_path, self.stage, self.long_short_strategy)

        crops_gen = convert_examples_to_crops(examples_gen=examples_gen,
                                              ...
                                              mask_padding_with_zero=self.mask_padding,
                                              p_keep_impossible=self.p_keep_impossible,
                                              sep_token_extra=self.sep_token_extra)

        for crop_index, crop in enumerate(crops_gen):

            answer_start = crop.start_position if self.is_training or self.is_validation else None
            answer_end = crop.end_position if self.is_training or self.is_validation else None
